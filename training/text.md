...
> $N$个集合，每个集合有若干整数，每次查询是否存在一个集合同时包含$a, b$两个整数。$N \leq 1000, Q \leq 200000, a_i \leq 10000$.

*solution:*

> 对每个整数维护包含他的所有集合，将1000个整数压入18个64位整数中。查询时枚举18个压位后的集合。

## URAL 1856 War and Peace

*problem:*

> N个点，初始每个点有一个人。每次可以进行两种操作(A, B)并给定每个点的(A, i, j)和(B, i, j)，表示在A/B操作下第i个点的人会移动到顶点j。如今Alice和Bob轮流操作，Alice要将所有顶点的人汇集到一个点，Bob的目标是阻止Alice。问最终谁获胜。N < 1000。

*solution:*

> 考虑剩下两个顶点时，Bob能胜利仅当这两个点不能被合并。由于初始状态下每个点都有人，因此充要条件就是一旦存在这样的点对(x, y)，无论如何都有办法让他们无法合并，则Bob获胜。因此考虑dp[a, b, 0/1]表示当前两个点为a和b，先/后手能否达到目标。由于转移可能出现环，因此可以这么考虑：从结束状态(x, x, 0/1)开始向外扩展，最终如果存在(a, b, 0)没被扩展到则Bob获胜。
